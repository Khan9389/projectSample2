...
public class PageBounds extends RowBounds implements Serializable {
    ...
    public PageBounds(PageInfo pageInfo) {
    	
    	Integer pageNo = pageInfo.getInt("pageNo");
		if(pageNo == null) {
			throw new UserBizException("페이지번호가 존재하지 않습니다.");
		}
		
		List<PentasMap> orders = pageInfo.getPentasMapList("orders");
		if(orders == null) {
			throw new UserBizException("페이지 정렬기준이 존재하지 않습니다.");
		}
		
		List<Order> orderlist = new ArrayList<Order>();
		for(PentasMap order : orders) {
			String target = order.getString("target");
			if(StringUtil.isNotEmpty(target.trim())) {
				orderlist.add(Order.create(order.getString("target"), order.getBoolean("isAsc", true) ? "ASC" : "DESC"));
			}
		}
		
		Integer limit = pageInfo.getInt("limit");
		if(limit == null) {
